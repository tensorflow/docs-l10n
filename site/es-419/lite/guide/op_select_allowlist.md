# Operadores TensorFlow seleccionados compatibles

Atención: La lista de operadores se actualiza con frecuencia.

## Operadores básicos de TensorFlow

La siguiente es una lista exhaustiva de las operaciones básicas de TensorFlow que son soportadas por el runtime de TensorFlow Lite con la función de Ops de TF seleccionados.
...
Los siguientes operadores [TensorFlow Text](https://www.tensorflow.org/tutorials/tensorflow_text/intro) y [SentencePiece](https://github.com/google/sentencepiece) son compatibles si usa la API de Python para la conversión e importa esas librerías.

Operadores de texto TF:
...
Por el lado del runtime, también es necesario vincular la librería TensorFlow Text o SentencePiece en la app o binario final.

## Operadores definidos por el usuario

*Nota: Esta característica sólo está disponible a partir de la versión 2.5 de TensorFlow*

Si [creó sus propios operadores TensorFlow](https://www.tensorflow.org/guide/create_op), también puede convertir los modelos que los contengan a TensorFlow Lite enumerando los operadores necesarios en el `experimental_select_user_tf_ops` como se indica a continuación:
...
Por el lado del runtime, también es necesario vincular su librería de operadores en la app o binario final.

## Añada los operadores básicos de TensorFlow a la lista permitida.

Si se da el caso de que los operadores básicos de TensorFlow no se encuentran en la [lista](https://www.tensorflow.org/lite/guide/op_select_allowlist#tensorflow_core_operators) permitida anteriormente, puede reportar la solicitud de característica [aquí](https://github.com/tensorflow/tensorflow/issues) con los nombres de los operadores básicos de TensorFlow, no listados en la lista permitida.

También puede crear su propio pull request desde el código fuente. Por ejemplo, si desea añadir el `raw_ops.StringToNumber` op en la lista permitida, hay tres lugares a actualizarse similar a este [commit](https://github.com/tensorflow/tensorflow/commit/02e691329517eb5e76522ed8d8bef79ceb082ff8).

(1) Añada el código fuente del kernel del operador a la regla BUILD `portable_extended_ops_group2`.
...
Para encontrar el archivo fuente del kernel del operador correspondiente en el directorio `tensorflow/core/kernels`, puede buscar en la ubicación del código fuente, que incluye la siguiente declaración del kernel con el nombre del operador:
...
Si hay algún archivo de cabecera bajo el directorio `tensorflow/core/kernels`, requerido en el código fuente del kernel del operador, necesita añadir el archivo de cabecera en la regla BUILD `portable_extended_ops_headers` como se indica a continuación:
...
(2) Añada el nombre del operador a la lista de permitidos.

La lista permitida está definida en `tensorflow/lite/delegates/flex/allowlisted_flex_ops.cc`. Es necesario listar el nombre del operador básico de TensorFlow para que se permita a través de la opción Select TF.
...
Como la lista anterior está ordenada por orden alfabético, se asegura de colocar el nombre en el lugar correcto.

(3) Añada el nombre del operador a esta página de la guía.

Para que el resto de desarrolladores sepan que el operador está incluido, esta página de la guía debe actualizarse también. Esta página se encuentra en `tensorflow/lite/g3doc/guide/op_select_allowlist.md`.
