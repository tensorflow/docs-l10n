...
根据经验作出的猜测是，此问题是由数值不稳定（如 NaN 或无穷大）引起的。但是，我们该如何确定这就是原因，以及如何找到产生数值不稳定的 TensorFlow 算子呢？为了回答这些问题，我们使用 Debugger V2 检测一下这个包含错误的程序。
...
至此，我们观察了 TF2 模型错误，并提出了修复该错误的代码更改建议。这是在 Debugger V2 工具的帮助下完成的，该工具提供了对所检测的 TF2 程序的 Eager 和计算图执行历史记录的完全可见性，包括张量值的数值摘要，以及算子、张量及其原始源代码之间的关联。
...
目前支持的 tensor_debug_modes 包括：`NO_TENSOR`、`CURT_HEALTH`、`CONCISE_HEALTH`、`FULL_HEALTH` 和 `SHAPE`。它们从每个张量提取的信息量以及所调试程序的性能开销各不相同。请参阅 `enable_dump_debug_info()` 文档的[“参数”部分](https://www.tensorflow.org/api_docs/python/tf/debugging/experimental/enable_dump_debug_info)。
