...
*   `TFLiteConverter.from_saved_model()`：用来转换
    [SavedModel 格式模型](https://tensorflow.google.cn/guide/saved_model)。
...
*   `TFLiteConverter.from_concrete_functions()`：用来转换
    [concrete functions](https://tensorflow.google.cn/guide/concrete_function)。

注意: 在 TensorFlow Lite 2.0 中有一个不同版本的
`TFLiteConverter` API， 该 API 只包含了
[`from_concrete_function`](https://tensorflow.google.cn/versions/r2.0/api_docs/python/tf/lite/TFLiteConverter#from_concrete_function)。
...
以下示例展示了如何将一个
[SavedModel](https://tensorflow.google.cn/guide/saved_model) 转换为
...
以下示例展示了如何将一个
[tf.keras 模型](https://tensorflow.google.cn/guide/keras/overview) 转换为
TensorFlow Lite 中的 [`FlatBuffer`](https://google.github.io/flatbuffers/) 格式。
...
[concrete function](https://tensorflow.google.cn/guide/concrete_function)
...
[pip](https://tensorflow.google.cn/install/pip) (推荐方式) 或
[Docker](https://tensorflow.google.cn/install/docker), 或
[从源代码构建 pip 包](https://tensorflow.google.cn/install/source).
