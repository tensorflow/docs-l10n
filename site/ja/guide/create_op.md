...
- C++ にある程度精通していること
- [TensorFlow バイナリ](https://www.tensorflow.org/install)がインストール済みであるか、[TensorFlow のソースコードがダウンロード済み](https://www.tensorflow.org/install/source)で、ビルドできること
...
データ検証によってコードの可読性を高め、デバッグしやすくし、保守しやすく、より堅牢にするには、演算子タイプのカスタム勾配を登録する際に、型ヒントを追加してください。たとえば、関数のパラメーターとして `op` を取る場合、勾配関数がパラメーターの型として <a href="https://www.tensorflow.org/api_docs/python/tf/Operation"><code>tf.Operation</code></a> を取ることを指定します。
