...
`TfLiteContext` と <code>TfLiteNode</code> の詳細については、[<code>common.h</code>](https://github.com/tensorflow/tensorflow/blob/master/tensorflow/lite/c/common.h) をご覧ください。前者はエラー報告ファシリティと、すべてのテンソルを含むグローバルオブジェクトへのアクセスを提供し、後者は実装が入力と出力にアクセスできるようにします。
